...
from pvprototypes_faers.dataops.utils_pandas import validate_schema
...
class FaersFilesCorrections(Processor):
    ...
    __FILES_WITH_ENCODING_ISSUES = ["drug19q3.txt"]
    __REPLACEMENTS = {
    ...
    __FILE_RENAMES = {"demo18q1_new.txt": "demo18q1.txt"}
    ...
    __COLUMN_DROPS = {"demo": ["foll_seq", "image", "death_dt", "confid"]}
    __COLUMN_RENAMES = {
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self._ts = int(datetime.now(tz=timezone.utc).timestamp())
        self._constants = Constants()
        logger.info("[__init__|out]")

    def process(self, context: Optional[Dict[str, Any]] = None) -> Any:
        """
        this method reads a faers file and applies fixes to it, based on provided configuration
        as, for instance, changing its name, changes some lines, fixes encoding, etc...
        in the end it loads it as a dataframe, applies further processing and checks the final schema,
        then saves the file back, it's meant to be idempotent
        """
        ...
        if context is None or self.CONTEXT_KEY_FILE not in context:
            raise Exception(f"you must provide context for {self.CONTEXT_KEY_FILE}")

        if self.CONTEXT_KEY_OUTPUT_FOLDER not in context:
            raise Exception(f"you must provide context for {self.CONTEXT_KEY_OUTPUT_FOLDER}")
        ...
        if table in self._constants.TABLES:
            ...
            if file in self.__FILE_RENAMES:
                ...
                f_after = os.path.join(file_folder, file)
                logger.debug(f"[__fix_file] renaming {filepath} to {f_after}")
                os.rename(filepath, f_after)
                filepath = f_after
                file_folder = os.path.dirname(filepath)
            ...
            if pattern_match is not None:
                ...
                year = int(str(datetime.now(tz=timezone.utc).year)[:2] + fileparts[0])
    ...
    def __change_line(self, file_name, line_num, replace: tuple[str, str]):
        """
    ...
    def __restore_file_ignoring_encoding_issues(self, filename):
        """
        ...
        lines = open(filename, mode="r", errors="ignore").readlines()
        with open(filename, "w") as f:
            f.writelines(lines)
        logger.debug("[restore_file_ignoring_encoding_issues|out]")

    def __validate_schema(self, table: str, df: pd.DataFrame):
        """
        ...
        """
        logger.debug(f"[validate_schema|in] ({table}, {df.shape})")

        if table in self.__SCHEMAS:
            expected_schema = self.__SCHEMAS[table]
            validate_schema(df, expected_schema)

        logger.debug("[validate_schema|out]")
